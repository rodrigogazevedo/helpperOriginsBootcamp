...
let list4 : {id: number, name: string, bio: string}[] = [
...
function alterImperativa(id: number): void {
...
function alterFunctional(id: number): void {
    console.log('O que deseja alterar: name ou bio?');
    const parameter: string = teclado();
    console.log("Digite o  que será inserirdo nesse campo a ser alterado");
    const value: string = teclado();
    list4.map(function (person): void {
        if (person.id === id) {
            if (parameter === 'bio') {
                person.bio = value;
            }
